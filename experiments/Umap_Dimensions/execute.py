...
def load_mega(data_dir: Path, datasets: List[str] = None, concat_train_validation: bool = True, label_columns: str = "standard activity code"):
    mega_dset = MegaHARDataset_BalancedView20Hz(data_dir, download=False)
    train_data, test_data = mega_dset.load(concat_train_validation=concat_train_validation, label=label_columns)
...
def _run(root_data_dir: str, output_dir: str, config: ExecutionConfig):
    ...
    output_file = output_dir / config.experiment_name / str(config.run_id) / file_id
...
if __name__ == "__main__":
    ...
    pool = Pool()
    results = pool.map(
        run, [(args.data_path, args.output_path, e) for e in experiments[:100]]
    )
